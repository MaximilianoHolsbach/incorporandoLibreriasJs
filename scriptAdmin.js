...
const arraycontenidos = JSON.parse(localStorage.getItem('contenidos'))??[]//Utilizando el operador nullish para consultar la existencia de la key del localStorage. 
...
formularioId.addEventListener('submit',(event)=>{ //Inicializo el evento de escucha para el formulario
    ...
    Swal.fire({//Libreria para avisar al administrador que el contenido fue agregado. 
        icon: 'success',
        title: 'Excelente!!!',
        text: 'Contenido agregado!',
      })
})
...
mostrarContenidos.addEventListener('click',()=>{
    ...
    cardContenido.forEach((contenido,index)=>{
        ...
        tarjetaContenido.children[1].children[2].addEventListener('click',()=>{//LLamo al segundo hijo de tarjetaContenido, y dentro del mismo busco el segundo hijo, y le asigno un evento click
            Swal.fire({//Libreria para consultar al administrador si realmente va a eliminar el contenido. 
                icon: 'warning',
                title: 'ATENCION!!!',
                showDenyButton: true,
                showCancelButton: true,
                html:`Vas a eliminar ${contenido.nombre} definitivamente`,
